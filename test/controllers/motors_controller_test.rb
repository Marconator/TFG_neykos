...
class MotorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create motor" do
    assert_difference('Motor.count') do
      post motors_url, params: { motor: { name: @motor.name, gateway_status_id: @motor.gateway_status_id, status: @motor.status } }
    end
  ...
  test "should update motor" do
    patch motor_url(@motor), params: { motor: { name: @motor.name, gateway_status_id: @motor.gateway_status_id, status: @motor.status } }
